...
typedef bool_t (FASTCALL *PFN_Bind) (bnode_t);
...
bnode_t bnOp;       // based node pointer when binding the right side of
                    // ., ->,
// Bind dispatch table
//SEGBASED(_segname("_CODE"))
bool_t (FASTCALL *pBind[]) (bnode_t) = {
    ...
    if (BindLChild (bn) && BindRChild (bn)) {
        if ((pExState->state.fSupOvlOps == FALSE) && EVAL_IS_CLASS (STP) &&
          (CLASS_PROP (STP).ovlops == TRUE)) {
            ...
            if (ValidateNodes ((op_t)OP_lbrack, STP, ST) && BDPlusMinus ((op_t)OP_plus)) {
                return (Fetch ());
            }
        }
    ...
    )
{
    ...
    if (!ValidateNodes ((op_t)OP_bang, ST, NULL)) {
        return (FALSE);
    }
    ...
    )
{
    ...
    if (retval == FALSE) {
        ...
        if (rewriteInProgress == FALSE &&
            ...
            NODE_OP(bn) = (op_t)OP_pointsto;
            NODE_STYPE(bn) = NODE_STYPE(bL);
            bn->v[0] = bL->v[0];

            NODE_OP(bL) = (op_t)OP_bscope;
...
{
    register op_t        nop = (op_t)OP_plus;
    ...
    )
{
    ...
    if (!BindLChild (bn) || !BindRChild (bn) || !ValidateNodes((op_t)OP_segop, STP, ST)) {
        return(FALSE);
    }
    ...
    )
{
    ...
    if (!ValidateNodes ((op_t)OP_addrof, ST, NULL))
        return (FALSE);
    ...
    )
{
    ...
    if (!ValidateNodes ((op_t)OP_fetch, ST, NULL)) {
        return(FALSE);
    }
...
struct _OvlMap  BinaryOvlMap[] = {
    {(op_t)OP_preinc      ,(op_t)OP_Oincrement  },
    {(op_t)OP_predec      ,(op_t)OP_Odecrement  },
    {(op_t)OP_postinc     ,(op_t)OP_Oincrement  },
    {(op_t)OP_postdec     ,(op_t)OP_Odecrement  },
    {(op_t)OP_function    ,(op_t)OP_Ofunction   },
    {(op_t)OP_lbrack      ,(op_t)OP_Oarray      },
    {(op_t)OP_pmember     ,(op_t)OP_Opmember    },
    {(op_t)OP_mult        ,(op_t)OP_Ostar       },
    {(op_t)OP_div         ,(op_t)OP_Odivide     },
    {(op_t)OP_mod         ,(op_t)OP_Opercent    },
    {(op_t)OP_plus        ,(op_t)OP_Oplus       },
    {(op_t)OP_minus       ,(op_t)OP_Ominus      },
    {(op_t)OP_shl         ,(op_t)OP_Oshl        },
    {(op_t)OP_shr         ,(op_t)OP_Oshr        },
    {(op_t)OP_lt          ,(op_t)OP_Oless       },
    {(op_t)OP_lteq        ,(op_t)OP_Olessequal  },
    {(op_t)OP_gt          ,(op_t)OP_Ogreater    },
    {(op_t)OP_gteq        ,(op_t)OP_Ogreatequal },
    {(op_t)OP_eqeq        ,(op_t)OP_Oequalequal },
    {(op_t)OP_bangeq      ,(op_t)OP_Obangequal  },
    {(op_t)OP_and         ,(op_t)OP_Oand        },
    {(op_t)OP_xor         ,(op_t)OP_Oxor        },
    {(op_t)OP_or          ,(op_t)OP_Oor         },
    {(op_t)OP_andand      ,(op_t)OP_Oandand     },
    {(op_t)OP_oror        ,(op_t)OP_Ooror       },
    {(op_t)OP_eq          ,(op_t)OP_Oequal      },
    {(op_t)OP_multeq      ,(op_t)OP_Otimesequal },
    {(op_t)OP_diveq       ,(op_t)OP_Odivequal   },
    {(op_t)OP_modeq       ,(op_t)OP_Opcentequal },
    {(op_t)OP_pluseq      ,(op_t)OP_Oplusequal  },
    {(op_t)OP_minuseq     ,(op_t)OP_Ominusequal },
    {(op_t)OP_shleq       ,(op_t)OP_Oleftequal  },
    {(op_t)OP_shreq       ,(op_t)OP_Orightequal },
    {(op_t)OP_andeq       ,(op_t)OP_Oandequal   },
    {(op_t)OP_xoreq       ,(op_t)OP_Oxorequal   },
    {(op_t)OP_oreq        ,(op_t)OP_Oorequal    },
    {(op_t)OP_comma       ,(op_t)OP_Ocomma      }
};
    ...
    )
{
    ...
    NODE_OP (EndArg) = (op_t)OP_endofargs;
    NODE_LCHILD (bn) = Fcn;
    NODE_RCHILD (bn) = 0;
    NODE_OP (bn) = (op_t)OP_noop;
...
struct _OvlMap UnaryOvlMap[] = {
    {(op_t)OP_bang        ,(op_t)OP_Obang       },
    {(op_t)OP_tilde       ,(op_t)OP_Otilde      },
    {(op_t)OP_negate      ,(op_t)OP_Ominus      },
    {(op_t)OP_uplus       ,(op_t)OP_Oplus       },
    {(op_t)OP_fetch       ,(op_t)OP_Ostar       },
    {(op_t)OP_addrof      ,(op_t)OP_Oand        },
};
    ...
    )
{
    ...
    NODE_OP (Right) = (op_t)OP_endofargs;
    NODE_LCHILD (bn) = Fcn;
    NODE_RCHILD (bn) = 0;
    NODE_OP (bn) = (op_t)OP_noop;
    ...
    NODE_OP (Left) = Oper;
    NODE_RCHILD (Fcn) = Right;
    NODE_OP (Right) = (op_t)OP_arg;
    NODE_LCHILD (Right) = NODE_LCHILD (bn);
    NODE_RCHILD (Right) = RightRight;
    NODE_OP (RightRight) = (op_t)OP_endofargs;
    NODE_LCHILD (bn) = Fcn;
    NODE_RCHILD (bn) = 0;
    NODE_OP (bn) = (op_t)OP_noop;
    ...
    )
{
    ...
    if ((op >= OP_this) && (op <= (op_t)OP_Odelete)) {
