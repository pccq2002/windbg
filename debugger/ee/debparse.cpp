...
// #undef OPCNT
	...
	0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 
	0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 
	0x1F, 0x1F, 0x1F, 0x1F, 0x1F, 0x00, 0x1D, 0x00, 0x1D, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 
	0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1B, 0x1F, 0x03, 0x03, 0x03, 0x1B, 0x00, 0x1B, 0x01, 0x1F, 0x1D, 
	0x00, 0x1D, 0x1D, 0x1D, 0x1D, 0x1D, 0x19, 0x19, 0x19, 0x19, 0x19, 0x17, 0x17, 0x15, 0x15, 0x13, 
	0x13, 0x13, 0x13, 0x11, 0x11, 0x0F, 0x0D, 0x0B, 0x09, 0x07, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 
...
// #undef OPCNT
		...
		0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 
			0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 
			0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 
			0x20, 0x20, 0x20, 0x20, 0x20, 0x1E, 0x00, 0x1E, 0x00, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 
			0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 0x1E, 0x04, 0x04, 0x04, 0x1C, 0x1C, 0x1C, 0x02, 0x1E, 0x00, 
			0x1C, 0x00, 0x1C, 0x1C, 0x1C, 0x1C, 0x1A, 0x1A, 0x18, 0x18, 0x18, 0x16, 0x16, 0x14, 0x14, 0x12, 
			0x12, 0x12, 0x12, 0x10, 0x10, 0x0E, 0x0C, 0x0A, 0x08, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06, 0x06, 
...
kludge:
        ...
        if (tokNext.opTok == OP_incr) {
            if (F[SRCUR().opTok] <= G[OP_incr]) {
                tokNext.opTok = (op_t)OP_preinc;
            }
            else {
                tokNext.opTok = (op_t)OP_postinc;
            }
        }
        else if (tokNext.opTok == OP_decr) {
            if (F[SRCUR().opTok] <= G[OP_decr]) {
                tokNext.opTok = (op_t)OP_predec;
            }
            else {
                tokNext.opTok = (op_t)OP_postdec;
            }
        }
        else if (tokNext.opTok == (op_t)OP_lcurly) {
          ...
          (tokOld.opTok == OP_rparen))) {
            ...
            tokNext.opTok = (op_t)OP_function;
        }
        else if ((tokNext.opTok == (op_t)OP_by || tokNext.opTok == (op_t)OP_wo ||
              tokNext.opTok == (op_t)OP_dw) &&
             (tokOld.opTok == (op_t)OP_dot || tokOld.opTok == (op_t)OP_pointsto)) {
            // by, wo, dw following a '.' or '->' are probably struct
            // members, not operators, so treat them as such (dolphin #977)
            tokNext.opTok = (op_t)OP_ident;
        }
        else if ((tokNext.opTok == (op_t)OP_lparen) ||
          ((tokNext.opTok == (op_t)OP_ident) &&
            (SRCUR().opTok == (op_t)OP_arg) && (fdepth > 0))) {
            ...
            switch (ParseTypeName (&tokNext, szExpr, radix, &error)) {
                ...
                case PTN_typestr:
                    if (SRCUR().opTok == OP_sizeof) {
                        ...
                        tokT = tokNext;
                        tokT.opTok = (op_t)OP_typestr;
                        if ((error = PushToken (&tokT)) != 0) {
                            break;
                        }
                        tokNext.opTok = (op_t)OP_cast;
                    }
                    break;
        ...
        if (tokNext.opTok == OP_rparen) {
            if ((SRCUR().opTok == OP_function) &&
              (argcnt[fdepth - 1] == 0)) {
                ...
                tokOld.opTok = (op_t)OP_endofargs;
                SRPUSH(tokOld);
                tokNext.opTok = (op_t)OP_fcnend;
                fdepth--;
            }
        ...
        if (F[SRCUR().opTok] <= G[tokNext.opTok]) {
            ...
            if((SRCUR().opTok == (op_t)OP_lparen) && (tokNext.opTok == OP_rparen)) {
    ...
    )
{
    if (ptokNext->opTok == OP_comma) {
        if (pdepth == 0) {
            ptokNext->opTok = (op_t)OP_lowprec;
        }
        else {
            if (ptokOld->opTok == (op_t)OP_arg) {
                // error if OP_arg followed immediately by a comma
                pExState->err_num = ERR_SYNTAX;
                return (EEGENERAL);
            }
            else {
                ptokNext->opTok = (op_t)OP_arg;
                argcnt[pdepth - 1]++;
            }
        }
    }
    ...
    switch (ptokOld->opTok) {
        ...
        case OP_typestr:
            ...
            switch (ptokNext->opTok) {
                case OP_fetch:
                    ptokNext->opTok = (op_t)OP_mult;
                    break;

                case OP_uplus:
                    ptokNext->opTok = (op_t)OP_plus;
                    break;

                case OP_negate:
                    ptokNext->opTok = (op_t)OP_minus;
                    break;

                case OP_addrof:
                    ptokNext->opTok = (op_t)OP_and;
                    break;

                case OP_uscope:
                    ptokNext->opTok = (op_t)OP_bscope;
                    break;

                case OP_bang:
                    if (ptokOld->opTok == OP_ident) {
                        ptokNext->opTok = (op_t)OP_execontext;
                    }
                    break;
            }
            break;
    ...
    )
{
    ...
    for (;;) {
        ...
        if ((state != PT_S0) || (ParenReq == TRUE)) {
           ...
           if (*szExpr != 0) {
               if ((GetToken ((uchar *)szExpr, &tokNext, radix, (op_t)OP_lowprec) != ERR_NONE) ||
                 (tokNext.opTok == OP_badtok)) {
                   pExState->err_num = ERR_SYNTAX;
                   *perror = EEGENERAL;
                   return (PTN_error);
    ...
    )
{
    ...
    for (szExpr = pExStr; *szExpr ; szExpr += tokNext.cbTok) {
        ...
        if (((GetDBToken ((uchar *)szExpr, &tokNext, radix, (op_t)OP_lowprec) != ERR_NONE) ||
            tokNext.opTok == OP_badtok)) {
    ...
    )
{
    ...
    ptokNext->opTok = (op_t)OP_context;
    ...
    )
{
    ...
    szExpr += pTok->cbTok;
    if (*szExpr &&
        GetDBToken (szExpr, &tokNext, radix, (op_t)OP_lowprec) == ERR_NONE &&
        tokNext.opTok == OP_lt) {
