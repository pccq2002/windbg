    ...
    )
{
    ...
    domatch:
        ...
        if (cmpflag && (pName->sstr.searchmask & SSTR_FuzzyPublic)) {
            char * ppatAlloc = (char *)MemAllocate(cbstName + 1);
            char * ppat = ppatAlloc;
            char * ptemp;
            BOOL doit = FALSE;
            ...
            if (doit && _tcslen(ppat) == pName->sstr.cb) {
                if (fCase == TRUE) {
                    cmpflag = _tcsncmp ((const char *)pName->sstr.lpName, ppat, pName->sstr.cb);
                }
                else {
                    cmpflag = _tcsnicmp ((const char *)pName->sstr.lpName, ppat, pName->sstr.cb);
                }
            }
