    ...
    *pval = 0L;
    ...
    for (;;) {
        ...
        *pval *= radix;
        ...
        pb = (uchar *) _tcsinc((const char *)pb);
    }
    pTok->pbEnd = (char *) pb;
    return (ERR_NONE);
}
    ...
    )
{
    ...
    VAL_LDOUBLE(pTok) = LdFromSz ((char *)pb, &pEnd);
#else
...
ulong  CanonOp (uchar *pb, ptoken_t pTok)
{
    ...
    if (_istalpha (*pb)) {
        ...
        *pOp++ = ']';
        ...
        for ( i = 0; i < OPCNT; i++) {
            if (_tcsncmp (OpStr[i].str + 1, (const char *) pb, OpStr[i].str[0]) == 0) {
                break;
            }
        }
        if (i == OPCNT) {
    ...
    )
{
    ...
    while ((*(TCHAR *)pb != _T('\'') ) && (*(TCHAR *)pb != 0)) {
        ...
        if ( c == _T('\\') ) {
            ...
            if ((retval = GetEscapedChar ((char **)&pb, &cbChar, &escVal)) != ERR_NONE) {
                return (retval);
            }
            c = (TCHAR)escVal;
        }
