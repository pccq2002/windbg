...
/**     MyEEParse - parse expression string to abstract syntax tree
 *
 *      ulong  MyEEParse (szExpr, radix, fCase, phTM);
 *
    ...
    )
{
    ...
    if ((pTMLbp->hTMList = MemAllocate (TMLISTCNT * sizeof (HTM))) != 0) {
        ...
        retval = MyEEParse (pExpr, radix, fCase, &pTMList[0], pEnd);
        pTMLbp->cTMListAct++;
            ...
            if ((retval = EEBindTM (&pTMList[0], SHpCXTFrompCXF (pCxf),
              TRUE, fEnableProlog)) != EENOERROR) {
                ...
                if ( pExState->err_num == ERR_NOVIRTUALBP )
                {
                    ...
                    {
                        retval = MyEEParse( funcName, radix, fCase, &pTMList[0], pEnd);
                        pTMLbp->cTMListAct = 1;
