...
typedef bool_t (FASTCALL *PFN_Eval) (bnode_t);

bool_t  FASTCALL    BindError (bnode_t bn);
bool_t  FASTCALL    BindPMember (bnode_t bn);
...
bool_t (NEAR FASTCALL *pEval[]) (bnode_t) = {
...
//            SHREG       spReg;
            ...
            if (fIncrementalEval &&
                ...
                NODE_OP (bnParent) = (op_t)OP_parent;
            }
...
bool_t FASTCALL EvalContext (bnode_t bn)
{
    ...
    if (error == TRUE) {
        ...
        if ((EVAL_STATE (ST) == EV_lvalue) && (EVAL_IS_BPREL (ST) || EVAL_IS_REGREL(ST) || EVAL_IS_TLSREL(ST)) &&
            ...
            if (EVAL_IS_REF (ST)) {
                if (!EvalUtil ((op_t)OP_fetch, ST, NULL, EU_LOAD)) {
...
// BUGBUG must support 64 bit constants
          ...
          (EVAL_STATE (ST) = EV_rvalue, EVAL_PTR (ST) = EVAL_SYM (ST), EVPlusMinus ((op_t)OP_plus) == TRUE)
          ) {
...
bool_t DerefVBPtr (CV_typ_t type)
{
    ...
    if (type != 0) {
        if (SetNodeType (ST, type) == TRUE) {
            EVAL_STATE (ST) = EV_lvalue;
            EVAL_SYM (ST) = EVAL_PTR (ST);
            if (!EvalUtil ((op_t)OP_fetch, ST, NULL, EU_LOAD | EU_TYPE)) {
                return (FALSE);
            }
...
bool_t FASTCALL Assign (op_t op)
{
    ...
    if (op == OP_eq) {
        ...
        if (!EvalUtil ((op_t)OP_eq, ST, NULL, EU_LOAD)) {
            return (FALSE);
        }
...
bool_t FASTCALL FetchOp (peval_t pv)
{
    ...
    if (!EvalUtil ((op_t)OP_fetch, pv, NULL, EU_LOAD | EU_TYPE)) {
        return (FALSE);
    }
...
bool_t FASTCALL EvalBang (bnode_t bn)
{
    ...
    if (!EvalUtil ((op_t)OP_bang, ST, NULL, EU_LOAD)) {
        return (FALSE);
    }
    if (EVAL_IS_REF (ST)) {
        if (FetchOp (ST) == FALSE) {
...
bool_t FASTCALL EvalArray (bnode_t bn)
{
    ...
    if (EvalLChild (bn) && EvalRChild (bn)) {
        if (EVAL_IS_ARRAY (STP) || EVAL_IS_ARRAY (ST)) {
            ...
            if (EvalUtil ((op_t)OP_lbrack, STP, ST, EU_LOAD) && EVPlusMinus ((op_t)OP_plus)) {
                return (FetchOp (ST));
            }
        }
        else if (EVAL_IS_PTR (STP)) {
...
bool_t FASTCALL EvalCast (bnode_t bn)
{
    ...
    if (!EvalRChild (bn) || !EvalUtil ((op_t)OP_cast, ST, NULL, EU_LOAD)) {
        return (FALSE);
    }
...
bool_t FASTCALL EvalSegOp (bnode_t bn)
{
    ...
    if (!EvalUtil ((op_t)OP_segop, STP, ST, EU_LOAD | EU_TYPE))
        return(FALSE);
        ...
        if (EVAL_STATE (ST) == EV_constant &&
            ...
            if (pa->flags.ref == TRUE) {
                ...
                if (EVAL_STATE (ST) == EV_constant) {
                    ...
                    if (!EvalUtil ((op_t)OP_function, ST, NULL, EU_LOAD)) {
                        goto EvalFunctionExit;
                    }
                    EVAL_PTR (ST) = EVAL_SYM (ST);
                }
...
/**     StoreP - store return value for  call
 *
 *      fSuccess = StoreP ();
 *
...
/**     PushOffset - push address value for  and fastcall
 *
...
//    DTI         dti;
    ...
    *pvT = *pv;
    ...
    if (!EvalUtil ((op_t)OP_fetch, pv, NULL, EU_LOAD)) {
        return (FALSE);
    }
    CLEAR_EVAL_FLAGS (pv);
...
INLINE bool_t VFuncAddress2 (peval_t pv, peval_t pvFunc)
{
    ...
    *pv = *pvThis;
    if (!SetNodeType (pv, FCN_VFPTYPE (pvFunc)) ||
        !EvalUtil ((op_t)OP_fetch, pv, NULL, EU_LOAD)) {
        return (FALSE);
    }
    CLEAR_EVAL_FLAGS (pv);
    ...
    while (TRUE) {
        *pvTmp = *pv;
        ...
        if (!SetNodeType (pvTmp, T_UCHAR) ||
            !EvalUtil ((op_t)OP_fetch, pvTmp, NULL, EU_LOAD)) {
            return (FALSE);
        }
