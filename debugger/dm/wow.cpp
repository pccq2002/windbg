...

{
    ...
    if ( !FVDMInitDone ) {
        ...
        if ( hmodVDM != (HANDLE)NULL ) {
            ...
            pfnVDMProcessException = (VDMPROCESSEXCEPTIONPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMProcessException" );
            pfnVDMGetPointer = (VDMGETPOINTERPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMGetPointer" );
            pfnVDMGetThreadSelectorEntry = (VDMGETTHREADSELECTORENTRYPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMGetThreadSelectorEntry" );
            pfnVDMGetThreadContext = (VDMGETCONTEXTPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMGetContext" );
            pfnVDMSetThreadContext = (VDMSETCONTEXTPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMSetContext" );
            pfnVDMGetSelectorModule = (VDMGETSELECTORMODULEPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMGetSelectorModule" );
            pfnVDMEnumProcessWOW = (VDMENUMPROCESSWOWPROC)
                GetProcAddress( (HMODULE)hmodVDM, "VDMEnumProcessWOW" );
    ...
    case DBG_SEGLOAD:
        lpb = (char *) lpdw[2];
        b = DbgReadMemory(hprcx, (UINT_PTR)lpb, &sn, sizeof(sn), (LPDWORD)&cbRead);
...

{
    ...
    for (i=0; i<hprcx->cDllList; i++) {
        if ((hprcx->rgDllList[i].fValidDll == FALSE) &&
            ...
            if ((hprcx->rgDllList[i].szDllName = (LPTSTR)MHAlloc(_tcslen(sz) + 1)) != NULL) {
