...
#ifndef KERNEL
    typedef NTSTATUS (WINAPI * QTHREAD)(HANDLE,THREADINFOCLASS,PVOID,ULONG,PULONG);
    ...
    )
{
    ...
    lpprl->lszRemoteExe = (LPTSTR)lpb;
    lpb += _tcslen((const char *)lpb) + 1;

    lpprl->lszCmdLine = (LPTSTR)lpb;
    lpb += _tcslen((const char *)lpb) + 1;

    lpprl->lszRemoteDir = (LPTSTR)lpb;
    lpb += _tcslen((const char *)lpb) + 1;
    ...
    while (chp) {
        if (chp != hprc) {
            ...
            if (!fKillRoot && (chp->pstate & ps_root)) {
                chp->pid    = (PID)-1;
                chp->pstate = (DMPSTATE) ((DWORD)ps_root | (DWORD)ps_destroyed);
...
//    HTHDX       hthdT;
    ...
    if (hthd->tstate & ts_dead) {
        ...
        if ( !(hthd->tstate & ts_frozen)) {
			...
			hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate & ~ps_deadThread);
        }
...
EXCEPTION_DESCRIPTION ExceptionList[] = {
    ...
    {(DWORD)DBG_CONTROL_C,                    efdStop,   (EXCEPTION_CONTROL)0, "Control-C"},
    {(DWORD)DBG_CONTROL_BREAK,                efdStop,   (EXCEPTION_CONTROL)0, "Control-Break"},
    {(DWORD)EXCEPTION_DATATYPE_MISALIGNMENT,  EFDDEFAULT, (EXCEPTION_CONTROL)0, "Datatype Misalignment"},
    {(DWORD)EXCEPTION_ACCESS_VIOLATION,       EFDDEFAULT, (EXCEPTION_CONTROL)0, "Access Violation"},
    {(DWORD)EXCEPTION_IN_PAGE_ERROR,          EFDDEFAULT, (EXCEPTION_CONTROL)0, "In Page Error"},
    {(DWORD)STATUS_ILLEGAL_INSTRUCTION,       EFDDEFAULT, (EXCEPTION_CONTROL)0, "Illegal Instruction"},
    {(DWORD)EXCEPTION_ARRAY_BOUNDS_EXCEEDED,  EFDDEFAULT, (EXCEPTION_CONTROL)0, "Array Bounds Exceeded"},
                // Floating point exceptions will only be raised if
                // the user calls _controlfp() to turn them on.
    {(DWORD)EXCEPTION_FLT_DENORMAL_OPERAND,   EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Denormal Operand"},
    {(DWORD)EXCEPTION_FLT_DIVIDE_BY_ZERO,     EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Divide by Zero"},
    {(DWORD)EXCEPTION_FLT_INEXACT_RESULT,     EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Inexact Result"},
    {(DWORD)EXCEPTION_FLT_INVALID_OPERATION,  EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Invalid Operation"},
    {(DWORD)EXCEPTION_FLT_OVERFLOW,           EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Overflow"},
    {(DWORD)EXCEPTION_FLT_STACK_CHECK,        EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Stack Check"},
    {(DWORD)EXCEPTION_FLT_UNDERFLOW,          EFDDEFAULT, (EXCEPTION_CONTROL)0, "Float Underflow"},
                // STATUS_NO_MEMORY can be raised by HeapAlloc and
                // HeapRealloc.
    {(DWORD)STATUS_NO_MEMORY,                 EFDDEFAULT, (EXCEPTION_CONTROL)0, "No Memory"},
    ...
    {(DWORD)STATUS_NONCONTINUABLE_EXCEPTION,  EFDDEFAULT, (EXCEPTION_CONTROL)0, "Noncontinuable Exception"},
    ...
    {(DWORD)STATUS_INVALID_DISPOSITION,       EFDDEFAULT, (EXCEPTION_CONTROL)0, "Invalid Disposition"},
    {(DWORD)EXCEPTION_INT_DIVIDE_BY_ZERO,     EFDDEFAULT, (EXCEPTION_CONTROL)0, "Integer Divide by Zero"},
    {(DWORD)EXCEPTION_INT_OVERFLOW,           EFDDEFAULT, (EXCEPTION_CONTROL)0, "Integer Overflow"},
    {(DWORD)EXCEPTION_PRIV_INSTRUCTION,       EFDDEFAULT, (EXCEPTION_CONTROL)0, "Privileged Instruction"},
    {(DWORD)STATUS_STACK_OVERFLOW,            EFDDEFAULT, (EXCEPTION_CONTROL)0, "Stack Overflow"},
    {(DWORD)STATUS_DLL_NOT_FOUND,             EFDDEFAULT, (EXCEPTION_CONTROL)0, "DLL Not Found"},
    {(DWORD)STATUS_DLL_INIT_FAILED,           EFDDEFAULT, (EXCEPTION_CONTROL)0, "DLL Initialization Failed"},
    {(DWORD)(0xE0000000 | 'msc'),             efdNotify, (EXCEPTION_CONTROL)0, "Microsoft C++ Exception"},
};
