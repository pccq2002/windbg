    ...
    )
{
    ...
    if (s &&
        ...
        lpBaseAddress < SavedBaseOfImage + s->VirtualAddress+s->SizeOfRawData &&
            ((s->Characteristics & (DWORD)IMAGE_SCN_CNT_CODE) ||
             (!s->Characteristics & (DWORD)IMAGE_SCN_MEM_WRITE))) {
    ...
    )
{
    if (AllocateDestinationString) {
        DestinationString->Buffer = (PCHAR)MHAlloc( DestinationString->MaximumLength );
    ...
    DestinationString->Length = (WORD)WideCharToMultiByte(
        CP_ACP,
        WC_COMPOSITECHECK,
        (const unsigned short *)SourceString->Buffer,
...
//    CHAR                        dir[_MAX_DIR];
    ...
    ii->Sections = (PIMAGE_SECTION_HEADER)MHAlloc( cb );
    ...
    )
{
    ...
    if (NT_SUCCESS(DmKdReadListEntry( vs.PsLoadedModuleList, &List ))) {
        ...
        if (NT_SUCCESS(DmKdReadLoaderEntry( DataTable, (PLDR_DATA_TABLE_ENTRY64)&DataTableBuffer ))) {
...
{
    DWORD      dwThreadId;
    LPSTR      szProgName = (LPSTR)MHAlloc( MAX_PATH );
    ...
    if (hprc) {
        ...
        for (hthdT = hprc->hthdChild; hthdT; hthdT = hthdT->nextSibling) {
            if ( !(hthdT->tstate & ts_dead) ) {
				...
				hthdT->tstate = (TSTATEX)((DWORD)hthdT->tstate | ts_dead);
				hthdT->tstate = (TSTATEX)((DWORD)hthdT->tstate & ~ts_stopped);
            }
        }
    }
        ...
        if (hprc->rgDllList[iDll].offBaseOfImage == dwBaseOfDll) {
            ...
            if (hprc->rgDllList[iDll].sec) {
                ...
                if (sig != IMAGE_NT_SIGNATURE) {
                    ...
                    sec = (PIMAGE_SECTION_HEADER)MHAlloc( nh.FileHeader.NumberOfSections * IMAGE_SIZEOF_SECTION_HEADER );
                    if (!sec) {
                        break;
                    }
...
SearchList:
    while (Next64 != vs.DebuggerDataList) {
        ...
        if (Header.OwnerTag == DataBlock->OwnerTag) {
            ...
            if (!CacheEntry) {
                CacheEntry = (PDEBUG_DATA_CACHE)malloc(SizeToRead - sizeof(DBGKD_DEBUG_DATA_HEADER64) + sizeof(DEBUG_DATA_CACHE));
            } else {
                RemoveEntryList(&CacheEntry->List);
            }
    ...
    switch( InputPig->ioctlSubType ) {
        ...
        case IG_READ_PHYSICAL:
            ...
            *phy = *(PPHYSICAL)InputPig->data;
            if (DmKdReadPhysicalMemory( (ULONG64)phy->Address, phy->Buf, phy->BufLen, &phy->BufLen )) {
                phy->BufLen = 0;
            }
            break;
        ...
        case IG_WRITE_PHYSICAL:
            phy = (PPHYSICAL) ReplyPig->data;
            *phy = *(PPHYSICAL)InputPig->data;
            if (DmKdWritePhysicalMemory( (ULONG64)phy->Address, phy->Buf, phy->BufLen, &phy->BufLen )) {
                phy->BufLen = 0;
            }
            break;
        ...
        case IG_KSTACK_HELP:
            ...
            SavedThread = 0;
            if (LoadKdDataBlock()) {
//                KdHelp->KiCallUserMode = (DWORD64)KdDebuggerData.KiCallUserMode;
...
//                KdHelp->KeUserCallbackDispatcher = (DWORD64)KdDebuggerData.KeUserCallbackDispatcher;
            }
    ...
    )
{
    ...
    // process the command
    //
    if (_tcsicmp( (const char *)lpsss->rgbData, "resync" ) == 0) {
       if ( !TryEnterCriticalSection(&csSynchronizeTargetInterlock) ) {
    ...
    if (_tcsicmp( (const char *)lpsss->rgbData, "cache" ) == 0) {
        ProcessCacheCmd(p);
        LpDmMsg->xosdRet = xosdNone;
    } else
    if (_tcsicmp( (const char *)lpsss->rgbData, "reboot" ) == 0) {
        if (TryApiLock()) {
    ...
    if (_tcsicmp( (const char *)lpsss->rgbData, "crash" ) == 0) {
        if (TryApiLock()) {
    ...
    if ( !_tcsicmp((const char *)lpsss->rgbData, "FastStep") ) {
        fSmartRangeStep = TRUE;
        LpDmMsg->xosdRet = xosdNone;
    } else
    if ( !_tcsicmp((const char *)lpsss->rgbData, "SlowStep") ) {
        fSmartRangeStep = FALSE;
        LpDmMsg->xosdRet = xosdNone;
    } else
    if ( !_tcsicmp((const char *)lpsss->rgbData, "trace") ) {
    ...
    )
{
    ...
    if (NewState == ts_running) {
		...
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate & ~(ts_stopped | ts_first | ts_second));
...
--*/
{
    ...
    cb = nh.FileHeader.NumberOfSections * IMAGE_SIZEOF_SECTION_HEADER;
    pSH = (PIMAGE_SECTION_HEADER)MHAlloc( cb );
    ReadMem( pSH, cb );
    ...
    for (j = 0; j < nDebugDirs; j++) {
        ...
        if (DebugDir.Type == IMAGE_DEBUG_TYPE_MISC) {
            ...
            pMisc = pT = (PIMAGE_DEBUG_MISC)MHAlloc(l);
...
//    DWORD                       i;
    ...
    if (iDll == hprc->cDllList) {
        ...
        //
        hprc->cDllList += 10;
        hprc->rgDllList = (PDLLLOAD_ITEM)MHRealloc(hprc->rgDllList,
    ...
    hprc->rgDllList[iDll].szDllName = (LPTSTR)MHAlloc(_ftcslen(fname)+_ftcslen(ext)+4);
...
//         dwStatus = DmKdReadMemoryWrapper((ULONG64)KdDebuggerData.MmSystemRangeStart,
