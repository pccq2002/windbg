...
// from kdapi.cpp
DWORD                       PollThreadId = 0;
KDDEBUGGER_DATA64           KdDebuggerData = {0};
DBGKD_GET_VERSION64         vs = {0};
BOOL                        fPacketTrace = FALSE;
int                         fCrashDump = 0;
...

{
    ...
    if (CrashDump) {
        cbr = DmpReadMemory( (LPVOID)qwOffset, lpb, cb );
        if (lpRead) {
            *lpRead = cbr;
        }
        return (cbr > 0) || (cbr == cb);
    ...
    switch (dwType) {
        ...
        case QT_RELOAD_MODULES:
            if (!ReloadStruct.Flag) {
                ...
                ReloadStruct.String = (PTCHAR)malloc(dwLen);
                _tcscpy( ReloadStruct.String, (const char *)dwData);
                ReloadStruct.Flag = 1;
            }
            break;
...
--*/
{
    ...
    pcs = (PCALLSTRUCT)malloc(sizeof(*pcs));
...
--*/
{
    ...
    lpSuspendThread = GetProcAddress((HMODULE)hdll, "SuspendThread");
        ...
        case ssvcFiberDebug:
            {
                ...
                Actvfbrs = (LPVOID *)malloc(iAfbrs*sizeof(LPVOID));
    ...
    )
{
    ...
    if ((hprc->pstate & ps_dead) || (hprc->rwHand == (HANDLE)INVALID)) {
		...
		hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate | ps_killed);
    ...
    if ( lpnlg->fEnable ) {
        ...
        if ( !hnlg ) {
            ...
            */
            ...
            LPNLG   lpnlgT = (LPNLG)LLLock ( hnlg );
...
--*/
{
    ...
    if ((hModule = LoadLibrary("psapi.dll")) != NULL) {
        LPFNGETNAME ProcAddr = (LPFNGETNAME)GetProcAddress((HMODULE)hModule, "GetModuleFileNameExA");
...

{
    ...
    *szModName = CMODULEDEMARCATOR;
    ...
    if (hprc->rgDllList[iDll].fValidDll) {
        ...
        if (CrashDump) {
            CopyFileNameFromDBGFileName( szModName+1, (PTCHAR)ldd->lpImageName );
            if ((hprc->rgDllList[iDll].szDllName = (LPTSTR)MHAlloc(_tcslen(szModName+1) + 1)) != NULL) {
                             ...
                             (LPDWORD) &cb))
        {
            ...
            if ((hprc->rgDllList[iDll].szDllName = (LPTSTR)MHAlloc(_tcslen(lpsz) + 1)) != NULL)
            {
            ...
            // serene...
            _ftcscpy(szModName + 1, rgch);
            if ((hprc->rgDllList[iDll].szDllName = (LPTSTR)MHAlloc(_tcslen(rgch) + 1)) != NULL)
            {
    ...
    *lplpbPacket= (LPBYTE)(lpmdl=(LPMODULELOAD)MHAlloc(*lpcbPacket));
    ...
    for (iobj=0; iobj<cobj; iobj++) {
        ...
        if ((oleseg = GetOleSegType(rgSecHdr[iobj].Name)) != olenone) {
            ...
            ++hprc->colerg;
            if (hprc->rgolerg) {
                hprc->rgolerg = (POLERG)MHRealloc(hprc->rgolerg,
                                        hprc->colerg * sizeof(OLERG));
            }
            else {
                hprc->rgolerg = (POLERG)MHAlloc(hprc->colerg * sizeof(OLERG));
            }
