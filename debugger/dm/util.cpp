    ...
    )
{
    ...
    LPTSTR                  szValueRead = (LPTSTR)MHAlloc( cbValueRead * sizeof(TCHAR));
...
//    PIMAGE_SECTION_HEADER   psect;
    ...
    if (pdi->Sections == NULL) {
        pdi->Sections = (PIMAGE_SECTION_HEADER)MHAlloc(cobj * sizeof(IMAGE_SECTION_HEADER));
		if (!pdi->Sections)
		{
			__asm int 3;
		}
...
--*/
{
    ...
    if ( cbInput == 0 ) {
        ...
        LPRTP lprtp = (LPRTP)MHAlloc ( FIELD_OFFSET( RTP, rgbVar ) + cbInput );
...
{
    int l = _tcslen(s);
    LPTSTR p = (LPTSTR)MHAlloc(l + sizeof(TCHAR));
    _tcscpy(p, s);
    return p;
}
