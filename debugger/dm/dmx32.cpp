...
INT DmpGetCurrentProcessorMip( VOID )
{
	__asm int 3;
	return 0;
}

INT DmpGetCurrentProcessorAlp( VOID )
{
	__asm int 3;
	return 0;
}

INT DmpGetCurrentProcessorPPC( VOID )
{
	__asm int 3;
	return 0;
}
...

{
    ...
    DmpDumpBase = (char *)MapViewOfFile(
        MemMap,
        FILE_MAP_READ,
        0,
        0,
...

{
    ...
    if (UserModeDump) {
        if (DumpHeaderUser->MajorVersion >= 4) {
            *Exception = (PEXCEPTION_RECORD) ( DmpDumpBase +
                                               (DWORD)DumpHeaderUser->DebugEventOffset +
                                               (DWORD)(FIELD_OFFSET(DEBUG_EVENT, u.Exception.ExceptionRecord) ));
            DumpThread = (PCRASH_THREAD) ( DmpDumpBase + DumpHeaderUser->ThreadStateOffset );

        } else {
            *Exception = (PEXCEPTION_RECORD) ( DmpDumpBase + (DWORD)DumpHeaderUser->DebugEventOffset );
            DumpThread = NULL;
        }
    ...
    if (strcmp( DmpDumpBase+fsize-8, "DUMPREF" ) == 0) {
        char *fname = (char *)malloc( fsize-sizeof(DUMP_HEADER) );
        //
    ...
    *DmpHeader = DumpHeader;
    ...
    DmpPdePage = (PULONG)PageToLocation ((DumpHeader->DirectoryTableBase & ValidPfnMask) >> ValidPfnShift);
...

{
    ...
    if (DmpPdePage[PdeOffset] & ValidPteMask) {
        ...
        if (DmpPdePage[PdeOffset] & LargePageMask) {
            ...
            PtePage = (PULONG)PageToLocation(PteToPfn(DmpPdePage[PdeOffset]));
            if (PtePage == NULL) {
                return NULL;
            }
...

{
    ...
    __try {
        ...
        while ((Location = (PCHAR)VaToLocation(VirtualAddress)) && (Size > 0)) {
            if (OutBuffer) {
                *OutBuffer++ = *Location;
            }
            Size -= 1;
...

{
    ...
    __try {
        ...
        while (Size) {
            Location = (char *)VaToLocation( VirtualAddress );
            if (!Location) {
                break;
            }
...

{
    ...
    __try {
        ...
        while ((Location = (PCHAR)PhysicalToLocation(PhysicalAddress)) && (Size > 0)) {
            *OutBuffer++ = *Location;
            Size -= 1;
            BytesCopied += 1;
            PhysicalAddress += 1;
...

{
    ...
    __try {
        ...
        while (Size) {
            Location = (PCHAR)PhysicalToLocation( PhysicalAddress );
            if (!Location) {
                break;
            }
            VirtualProtect( Location, SystemInfo.dwPageSize, PAGE_WRITECOPY, &Protect );
...
#define DMP_DEBUG_EVENT 1
#define DMP_MEMORY_BASIC_INFORMATION 2
#define DMP_THREAD_CONTEXT 3
#define DMP_THREAD_STATE 4
#define DMP_MODULE 5
#define DMP_MEMORY_DATA 6
...
#error( "unknown target machine" );
    ...
    //
    DumpHeader.DebugEventOffset = (PUCHAR)SetFilePointer( hFile, 0, 0, FILE_CURRENT );
    ...
    //
    DumpHeader.MemoryRegionOffset = (DWORD)SetFilePointer( hFile, 0, 0, FILE_CURRENT );
    do {
        __try {
            rval = DmpCallback(
...
#endif
VOID WINAPI CleanupDisassembler(VOID) { }
//#ifdef KERNEL
...
--*/
{
    ...
    hprc->llnlg = LLInit(sizeof ( NLG ), llfNull, NULL, (LPFNFCMPNODE)NLGComp );
#endif
...
--*/
{
    ...
    hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate & (~ps_preStart));  // Clear the loading state flag
    hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate | (ps_preEntry));   // next stage...
    hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | (ts_stopped));  // Set that we have stopped on event
...
--*/
{
	...
	hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate & (~ps_preStart));
	hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate | (ts_stopped));// Set that we have stopped on event
    --nWaitingForLdrBreakpoint;
...
--*/
{
	...
	hthdx->hprc->pstate = (DMPSTATE)((DWORD)hthdx->hprc->pstate & (~ps_preEntry));
...
--*/
{
    ...
    if (hthd->hprc->f16bit) {
		...
		hthd->hprc->pstate = (DMPSTATE)((DWORD)hthd->hprc->pstate & (~ps_preEntry));
        //hthd->tstate |= ts_stopped;
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | (ts_stopped));
...

{
    ...
    if (hthd) {
        ...
        //hthd->tstate |= ts_stopped;
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate & (~ts_running));
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | (ts_stopped));
    }
    ...
    if (eventCode == EXCEPTION_DEBUG_EVENT) {
        ...
        case EXCEPTION_SINGLE_STEP:
            ...
            DecodeSingleStepEvent( hthd, de, &eventCode, (PDWORD_PTR)&subClass );
            break;
...

{
    ...
    if (hthd) {
        //hthd->tstate &= ~ts_running;
        //hthd->tstate |= ts_stopped;
		hthd->tstate = (TSTATEX)((DWORD)hprc->pstate & ~ts_running);
		hthd->tstate = (TSTATEX)((DWORD)hprc->pstate | ts_stopped);
    }
...
--*/
{
    ...
    case EXCEPTION_DEBUG_EVENT:
        if (subClass!=EXCEPTION_SINGLE_STEP){
            ...
            LPEPR lpepr  = (LPEPR)MHAlloc(nBytes);
    ...
    case LOAD_COMPLETE_DEBUG_EVENT:
        {
            LPBPR lpbpr = (LPBPR)MHAlloc ( sizeof ( BPR ) );
#ifdef TARGET_i386
    ...
    case CREATE_PROCESS_DEBUG_EVENT:
         ...
         */
        {
             ...
             */
                ...
                LPNPP lpnpp = (LPNPP)MHAlloc(cbPacket=sizeof(NPP));
    ...
    case EXIT_PROCESS_DEBUG_EVENT:
		...
		hthd->hprc->pstate = (DMPSTATE)((DWORD)hthd->hprc->pstate | ps_exited);
        rtp.hpid    = hthd->hprc->hpid;
        rtp.htid    = hthd->htid;
        rtp.dbc = dbcProcTerm;
        break;
    ...
    case EXIT_THREAD_DEBUG_EVENT:
		...
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | ts_dead);
		hthd->hprc->pstate = (DMPSTATE)((DWORD)hthd->hprc->pstate | ps_deadThread);
    ...
    case DESTROY_PROCESS_DEBUG_EVENT:
		...
		hthd->hprc->pstate = (DMPSTATE)((DWORD)hthd->hprc->pstate | ps_destroyed);
    ...
    case DESTROY_THREAD_DEBUG_EVENT:
		...
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | ts_destroyed);
        cbPacket    = sizeof(DWORD);
    ...
    case OUTPUT_DEBUG_STRING_EVENT:
        {
			...
			lpbPacket = (LPBYTE)MHAlloc(cbPacket);
            lpinf = (LPINFOAVAIL) lpbPacket;
            toFree = lpbPacket;
...
//
    ...
    if (!(rtp.cb=cbPacket)) {
        DmTlFunc((TLF)packetType, rtp.hpid, FIELD_OFFSET(RTP, rgbVar), (LPARAM) &rtp);
    }
    else {
        ...
        DmTlFunc((TLF)packetType, rtp.hpid, (FIELD_OFFSET(RTP, rgbVar)+cbPacket), (LPARAM) lprtp);
      ...
      case INPUT_DEBUG_STRING_EVENT:
        {
            ...
            _tcscpy((char *) de->u.DebugString.lpDebugStringData, abEMReplyBuf);
//
    ...
    )
{
	...
	hthd->hprc->pstate = (DMPSTATE)((DWORD)hthd->hprc->pstate | ps_dead);
    ...
    )
{
    ...
    if ( hthd->hprc->pstate & ps_preStart ) {
		...
		hthd->hprc->pstate = (DMPSTATE)((DWORD)hthd->hprc->pstate & ~ps_preStart);
        //hthd->tstate |= ts_stopped;
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | ts_stopped);
    ...
    default:
        ...
        if (!firstChance) {
			...
			hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | ts_first);
    ...
    )
{
    if (hthd) {
        //hthd->tstate |= ts_rip;
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | ts_rip);
    }
    NotifyEM( de, hthd, 0, 0 );
}                               /* ProcessRipEvent() */
    ...
    )
{
    ...
    if (!pbp) {
        ...
        if (pbp->bpType == bptpMessage) {
            ...
            if (!GetWndProcMessage (hthd, (UINT*)&msgi.dwMessage)) {
                assert (FALSE);
            }
...
--*/
{
    ...
    hprc->hthdChild     = (struct _HTHDX *)hthd;
...
--*/
{
    ...
    if (fUseRoot) {
        //hprc->pstate           |= ps_root;
		hprc->pstate            = (DMPSTATE)((DWORD)hprc->pstate | ps_root);
        hprc->hpid              = hpidRoot;
        fUseRoot                = FALSE;
    ...
    )
{
    ...
    //hprc->pstate |= ps_dead;
	hprc->pstate            = (DMPSTATE)((DWORD)hprc->pstate | ps_dead);
    ...
    )
{
    ...
    if (hthd->tstate & ts_frozen) {
		...
		hthd->tstate = (TSTATEX)((DWORD)hthd->tstate & ~ts_frozen);
    }
    ...
    )
{
    ...
    _ftcscpy( nameBuffer, (const char *)de.u.CreateProcessInfo.lpImageName);
	...
	hprc->pstate = (DMPSTATE)((DWORD)hprc->pstate | ps_connect);
    //
    ...
    if (64 == VersionInfo.PointerSize) {
        de = *(LPDEBUG_EVENT64)((PUCHAR)CrashDumpHeader + (DWORD)CrashDumpHeader->DebugEventOffset);
        //
...

{
    ...
    if (dosHdr.e_magic != IMAGE_DOS_SIGNATURE) {
        ...
        CloseHandle(hFile);
#ifndef KERNEL
        if (DmpInitialize( szAppName, &CrashContext, &CrashException, (PVOID *)&CrashDumpHeader )) {
            //
        ...
        if (hthd) {
            if (lpde->dwDebugEventCode == EXCEPTION_DEBUG_EVENT) {
                if (lpde->u.Exception.dwFirstChance) {
                    ...
                    //hthd->tstate |= ts_second;
					hthd->tstate = (TSTATEX)((DWORD)hthd->tstate | ts_second);
                }
            }
            //hthd->tstate &= ~ts_running;
            //hthd->tstate |= ts_stopped;
