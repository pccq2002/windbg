...
#include <simpldis.h>
#include "emdp.h"
#include "../disasm/ODDisasm/disasm.h"
...
int WINAPI MySimplyDisassemble(
							 PBYTE           pb,
							 const size_t    cbMax,
    ...
    if (GetAddrOff(*lpgpis->lpaddr) == 0) {
        ...
        *(lpgpis->lpuoffset) = GetAddrOff(*(lpgpis->lpaddr)) & 3;
        ...
        while (cbTry > 0) {
            ...
            while (1) {
				int Bytes = 0;
                Bytes = MySimplyDisassemble(pbCurr,
                                              cbCurr,
                                              (DWORD)currOffset,
                                              arch,
                                              &Sdis,
							 ...
							 )
{
	...
	SIMPLEDIS* MySdis = (SIMPLEDIS*)Sdis;
	t_disasm da;
	int nCodelen = 0;
	DWORD MyAddress = (DWORD)Address;

	ZeroMemory(MySdis, sizeof(SIMPLEDIS));
	MyAddress = ntohl(MyAddress);
	nCodelen= ODDisasm((char*)pb, cbMax,0, &da, DISASM_CODE);
	SprintfSuppliesLen(MySdis->szAddress, (PBYTE)&MyAddress, sizeof(DWORD));
	SprintfSuppliesLen(MySdis->szRaw, pb, nCodelen);
	strncpy(MySdis->szOpcode, da.result, SD_STRINGMAX);
