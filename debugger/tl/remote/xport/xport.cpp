...
--*/
{
    ...
    if (HDm) {
        DPRINT(("LoadDm: Already loaded, HDm == 0x%x\n", HDm));
        return xosdInUse;
    }

    HDm = LoadLibrary(lds->lpDmName);
    if (HDm == NULL) {
        DPRINT(("LoadDm: LoadLibrary failed\n"));
        xosd = xosdUnknown;
    } else
    ...
    if (HDm) {
        LpDmInit(DMTLFunc, (LPVOID) lds->lpDmParams);
    }
    ...
    )
{
    ...
    memcpy(PnlblkRq->rgchData, (char *) lds, sizeof(LOADDMSTRUCT));
    ...
    PnlblkRq->cchMessage = (WORD)(p - (LPSTR)(PnlblkRq->rgchData));
    ...
    )
{
    ...
    switch( pnlblk->mtypeBlk ) {
        ...
        case mtypeAsyncMulti:
            ...
            if (FConnected) {
                ...
                if (pMpacket->packetNum + 1 == pMpacket->packetCount) {
                    ...
                    if (TLCallBack != NULL) {
                        TLCallBack( pnlblk->hpid, (LPARAM) cb, (LPARAM) pbMulti);
        ...
        case mtypeDisconnect:
            ...
            if (TLCallBack != NULL) {
                RTP rtp = { dbcRemoteQuit, cb, pnlblk->hpid, 0, 0 };
        ...
        case mtypeLoadDM:
            ...
            memcpy(PnlblkRp->rgchData, (LPB) &xosd, sizeof(XOSD));
    ...
    )
{
    ...
    if (LpUiServer) {
        ...
        RTP rtp = { dbcRemoteQuit, 0, hpidRoot, 0, 0 };
...
{
    if (!TlWriteTransport(lpvOut, (DWORD) cbOut)) {
        return xosdWrite;
    }
