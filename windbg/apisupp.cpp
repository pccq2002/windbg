...
#pragma comment(lib,"debugger/em/Dbghelp.lib")
...
BOOL __stdcall SYSetProfileString(LPCSTR szName, LPCSTR szValue);
BOOL __stdcall SYGetProfileString(LPCSTR szName, LPSTR szValue, ULONG cbValue,
                        ULONG * pcbRet);
UINT
__stdcall
DHGetDebuggeeBytes(
...
//typedef BOOL     (WINAPI* PSYMBOLSERVERPROC)(PCSTR, PCSTR, ULONG, DWORD, DWORD, PSTR);
...
startDirectorySearch:
	PathTail[ Depth ] = (char *)strchr( (const char *)PathBuffer, '\0' );
    ...
    )
{
    ...
    if (!ghSrv) {
        ...
        gszSrvParams = (PCHAR) strchr((const char *) gszSrvName, '*');
        if (!gszSrvParams ) 
            return FALSE;
        *gszSrvParams++ = '\0';
    ...
    if (ghSrv == INVALID_HANDLE_VALUE)
		...
		strcpy(gszSrvName, &ServerInfo[7]);
		gszSrvParams = (PCHAR) strchr((LPCSTR) gszSrvName, '*');
	  	*gszSrvParams++ = '\0';
    
    return gfnSymbolServer(gszSrvParams, FileName, (PVOID)num1, num2, num3, FilePath);
}
    ...
    )
{
    ...
    p = (LPSTR) strchr( (LPCSTR) &FileName[2], '\\' );
    if (!p) {
        //
        // malformed name
        //
    ...
    )
{
	...
	//CHAR	SubDirPart[_MAX_DIR],
	CHAR FilePart[_MAX_FNAME], Ext[_MAX_EXT];
    BOOL symsrv=TRUE;
...
//                        goto next;
                } else if (MySearchTreeForFile( (PCHAR) DirectoryPath, FileName, ImageFilePath )) {
                                    ...
                                    PFIND_EXE_FILE_CALLBACK Callback, PVOID user)
{
    ...
    if (Callback) return INVALID_HANDLE_VALUE;
...
//    TCHAR                       szBuf[MAX_PATH];
    ...
    FindExecutableImageEx(szFileName,
                          pzSearchPath, 
                          rgch,
                          (PFIND_EXE_FILE_CALLBACK)FindDebugFileCallback, 
                          &FindSymFileData
                          );
    ...
    FindDebugInfoFileEx(szFileName, 
                        pzSearchPath, 
                        rgch,
                        (PFIND_DEBUG_FILE_CALLBACK)FindDebugFileCallback, 
                        &FindSymFileData
                        );
...
UINT __stdcall SYProcessor(VOID)
{
    long l = 0;
    ...
    )
{
    ...
    if (*szStr == '?') {
        if (MyUnDecorateSymbolName( szStr,
                                  szUndecStr,
                                  sizeof(szUndecStr),
                                  UNDNAME_COMPLETE                |
    ...
    )
{
	...
	//eest = EEGetHSYMList(&hsyml, &lpnsym->cxt, HSYMR_lexical, NULL, TRUE);
    if (eest != EENOERROR) {
        return FALSE;
    }
