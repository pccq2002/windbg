...
--*/
{
    ...
    while (!err) {
        ...
        else if (!fQuote &&  (!*lps || *lps == L' ' || *lps == L'\t' || *lps == L'\r' || *lps == L'\n'))
        {
            *lpt = '\0';
            break;
        }
    ...
    if (!Err) {
        ...
        if( !hTI  ||  !(pTI = (PTI) MMLpvLockMb (hTI)) ) {
            ...
            if( pTI->fResponse.fValue  &&  pTI->fResponse.Type == RIT.Type ) {
                memcpy (pValue, (char *) pTI->Value, (short) pTI->cbValue);
            } else {
                Err = BADTYPECAST;
            }
            MMbUnlockMb(hTI);
...
--*/
{
    ...
    } else if (!strchr( (LPCSTR) "iIlL", ch) || *(lpsz = CPSkipWhitespace(lpsz+1)) == '\0') {
