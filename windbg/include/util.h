...
int  MsgBox(HWND hwndParent, LPSTR szText, UINT wType);
...
BOOL  GetQCQPEnvStr(LPSTR lpszVar, LPSTR lpszDestEnv, WORD wDestSize);
...
HANDLE  LoadQCQPLibrary(LPSTR LibName, BOOL DoErrorBox);

// WinExec a prog (qcqp.exe path tried first)
WORD  WinQCQPExec(LPSTR CmdLine, int nCmdShow, BOOL DoErrorBox);
...
ULONG  StopTimeIn100ths(
    ULONG DelayIn100ths);

// Process a message received by QCQP
void  ProcessQCQPMessage(LPMSG lpMsg);
...
//To test if a character belong to the C/ Alphanumeric set
...
BOOL FAR  QueryCloseAllDocs(
    void);

BOOL FAR  StartFileDlg(HWND hwnd, int titleId, int defExtId,
...
void  EscapeAmpersands(
    LPSTR AmpStr,
    int MaxLen);

void  UnescapeAmpersands(
    LPSTR AmpStr,
    int MaxLen);

void  BuildTitleBar(LPSTR TitleBarStr, UINT MaxLen);

void  UpdateTitleBar(
    TITLEBARMODE Mode,
    BOOL Repaint);

void  DlgEnsureTitleBar(void);

// BUGBUG - dead code - kcarlos
#if 0
void FAR  FileNotSaved(int doc);
#endif

int FAR  ConvertPosX(int x);
