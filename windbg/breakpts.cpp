                ...
                if ( bpIpid == Ipid )  {
                    ...
                    if ( BPSymbolsMayBeAvailable( hBpt ) ) {
                        ...
                        if ( (bpstat != BPNOERROR) &&
                            ...
                            Dbg(BPNextHbpt( &hBpt, bptNext) == BPNOERROR);
                            EnsureFocusDebugger();
                            StartDialog(DLG_UNRESOLVED, (DLGPROC)DlgUnresolved);
                            if ( BpDlgCalled ) {
                                break;
                            }
                            continue;
    ...
    )
{
    ...
    if ( AutoTest ) {
        ...
        StartDialog( DLG_BPRESOLVE, (DLGPROC)DlgBpResolve );
    }
      ...
      case WM_INITDIALOG:
        ...
        if ( BpString ) {
            ...
            *Buffer = '\0';
            ...
            if ( *BpString != '{' || !(p = (PSTR) strchr( (LPCSTR) BpString+1, '}' )) ) {
                p = BpString;
            } else {
                c = *++p;
                *p = '\0';
        ...
        for (i = 0; i < (int)TmlBpResolve->cTMListAct; i++ ) {
            ...
            *rgch = '\0';
            ...
            if (eehstr) {
                ...
                if ( pch && strchr( (LPCSTR) pch,'{') ) {
                    char *p = (PSTR) strchr( (LPCSTR) pch, '}' );
                    if ( p ) {
                        if ( *rgch == '\0' ) {
                            char c;
                            c      = *(p+1);
    ...
    )
{
    ...
    BpString = BpBuf;
    StartDialog( DLG_BPCANIUSETHUNK, (DLGPROC)DlgBpCanIUseThunk );
      ...
      case WM_INITDIALOG:
        if ( BpString ) {
            ...
            *Buffer = '\0';
            ...
            if ( *BpString == '{' ) {
                if (p  = (PSTR) strchr( (LPCSTR) BpString+1, '}' ) ) {
                    p++;
                    c  = *p;
                    *p = '\0';
                    BPShortenContext( BpString, Buffer );
