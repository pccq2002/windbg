...
int __stdcall CLLookupAddress ( ADDR addr )
{
    int ifme;
...
char * __stdcall CLGetParams ( PHTM phtm, FRAME *pframe, int *pcbMax, char *pch )
{
    ushort      i = 0;
    ulong       cParm = 0;
    char FAR *  lpch;
        ...
        *pcbMax -= 1;
        ...
        for ( i = 0; i < cParm; i++ ) {
            ...
            if ( *pcbMax > 0 &&
                ...
                EEvaluateTM ( &htmParm, (HFRAME__ *)pframe, EEHORIZONTAL );
                if ( EEGetValueFromTM ( &htmParm, radix, (PEEFORMAT) "p", &hName ) ==
                      EENOERROR ) {
                    int ich;

                    lpch = (char*)MMLpvLockMb ( hName );
                    memcpy ( pch, lpch,
...
char * __stdcall CLGetProcName ( int ifme, char *pch, int cbMax, BOOL bSpecial )
{
    char *      pchT;
      ...
      case cltNone:
        {
            ...
            SYFixupAddr ( &addrT );
            EEFormatAddress( &addrT, pch, cbMax, g_contWorkspace_WkSp.m_bShowSegVal * EEFMT_SEG);
            len = strlen(pch);
            pchT += len;
            break;
...
MakeName:
        ...
        //
        if (bSpecial) {
            EEFormatCXTFromPCXT(&cxt, &hstr, 0);
        ...
        retval = EEGetTMFromHSYM ( symbol, &cxt, &htm, &strIndex, FALSE, FALSE );
        ...
        if ( EEGetNameFromTM ( &htm, &hName ) == EENOERROR ) {
            ...
            char far * lpch = (char far * )MMLpvLockMb ( hName );
        ...
        if ( ( cbMax > 0 ) && pfme->clt == cltProc ) {
            ...
            pchT = CLGetParams ( &htm, (struct _FRAME *)&pfme->hFrame, &cbMax, pchT );
...
*
*/
void __stdcall NEAR CLSetProcAddr ( FME *pfme, int *fInProlog )
{
    CXT  cxt;
    ADDR addrT;
...
void __stdcall CLGetWalkbackStack ( LPPD lppd, LPTD lptd )
{
//     STKSTR      stkStr;
...
PCXF LOADDS __stdcall CLGetFuncCXF ( PADDR paddr, PCXF pcxf )
{
    ADDR        addr;
    int         ifme;
    FME *       pfme = NULL;
