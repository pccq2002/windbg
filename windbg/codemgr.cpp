...
EESTATUS     MELoadEEParse(const char FAR *, EERADIX, SHFLAG, PHTM, LPDWORD);
...
VOID WINAPI    CmdMatchOpenedDocPrompt(BOOL, BOOL);
BOOL WINAPI    CmdMatchOpenedDocInputString(LPSTR);
...
void AdjustDebugLines(int DocNumber, int StartLine, int NumberLines, BOOL Added)
{
    Unused(DocNumber);
    Unused(StartLine);
    Unused(NumberLines);
...
#else
        memcpy ( TmpSrcFname, lpchFname + 1, *lpchFname);
    ...
    )
{
    ...
    if (Views[thisView].Doc < 0) {
        ...
        if (!DisasmGetAddrFromLine(&addr, Views[thisView].Y, &bSegAlwaysZero)) {
            return FALSE;
        }
    ...
    )
{
    ...
    if (Views[View].Doc < 0) {
        ...
        if (DisasmGetAddrFromLine(&addr, line, &bSegAlwaysZero)) {
            GoUntil(&addr);
            return TRUE;
        } else {
            return FALSE;
...
//    LPBPR       lpbpr;
    ...
    case dbcCheckBpt:
        ...
        bpstatus = BPCheckHbpt( Cxf, (LPFNBPCALLBACK)BPCallbackFunc, hpid, htid, wParam);
...
//  char *      szDllEEAuto;
    ...
    */
    ...
    Assert(NULL == strchr( (LPCSTR) pszDll, ' '));
    Assert(NULL == strchr( (LPCSTR) pszDll, '\t'));
    
#if 0
    if ((lpch = (PSTR) strchr( (LPCSTR) pszDll, ' ')) == NULL) {
        lpch = (PSTR) strchr( (LPCSTR) pszDll, '\t');
    }
...
BOOL     BPQuerySrcWinFls(char * pfls)
{
    Unreferenced( pfls );
    ...
    )
{
    ...
    return(MyEEParse(lpb, iRadix, shflag, phtm, lpus));//004E6660
}                   /* MELoadEEParse() */
...
BOOL   BPCBGetSourceFromAddr(LPADDR pAddr, char FAR * rgchFile, int cchFile, int FAR * pLine)
{
    DWORD    iLine;
...
matchopeneddocagain:
      ...
      StartDialog( DLG_FSRESOLVE, (DLGPROC)DlgFileSearchResolve );
      if (lMatchIdx >= 0) {
    ...
    case WM_COMMAND:
        ...
        case IDC_BUT_ADVANCED:
            DialogBox(g_hInst,
                      ...
                      hDlg,
                      (DLGPROC)DlgProc_Adv_BadSymbols);
            return TRUE;
...
browseagain:
    ...
    bs.DlgProc      = (DLGPROC)DlgBrowse;
    bs.pszFileName  = lpb;
    bs.FnameSize    = cb;

    hThread = CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE)BrowseThread, (LPVOID)&bs, 0, &id );
    if (!hThread) {
        return FALSE;
    }
    ...
    if (bs.Rslt == IDCANCEL) {
        ...
        return MiscBrowseForFile(lpb,
                                 ...
                                 SrcSetMapped,
                                 (LPOFNHOOKPROC)GetOpenFileNameHookProc,
...
--*/
{
    ...
    bs.DlgProc          = (DLGPROC)DlgProcBadSymbols;
    //bs.szFileName;
    //bs.FnameSize;
            ...
            case IDC_BUT_ADD:
                if (BN_CLICKED == wNotifyCode) {
                    ...
                    if (IDOK == DialogBoxParam(g_hInst, MAKEINTRESOURCE(IDD_EDIT_SRC_ROOT_MAPPING),
                        hwndDlg, (DLGPROC)DlgProc_EditRootMapping, (LPARAM) &mrp)) {
