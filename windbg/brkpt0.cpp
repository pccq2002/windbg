...
BPSTATUS  __stdcall BPBindHbptAddr( HBPT hBpt, LPBPP lpBpp, BOOL fOnList, CXF *pCxf );
BPSTATUS  __stdcall BPBindHbptNoAddr( HBPT hBpt, LPBPP lpBpp, BOOL fOnList, CXF *pCxf );
                  ...
                  )
{
    ...
    */
    ...
    if (*sz == OPENCONTEXTOP) {      /* check for a context operator here */
        ...
        lpBp = (LPSTR) strchr((LPCSTR) sz, CLOSECONTEXTOP);
        if (!lpBp) {
            return BPBadContextOp;
        }
        lpBp++;

        lp0 = (LPSTR) strchr((LPCSTR) sz, ',');
    ...
    } else if ( (lp0 = (LPSTR) strchr( (LPCSTR) sz, '!' )) ) {
                ...
                )
{
    ...
    *Exe    = NULL;
    ...
    if ( p = (PSTR) strchr( (LPCSTR) BpBuf, OPENCONTEXTOP ) ) {

        if ( p = (PSTR) strchr( (LPCSTR) p+1, ',' ) ) {
...
Routine Description:
    ...
    the nested languages issues later. (i.e. )
...

{
    ...
    *File = '\0';
    ...
    if ( BPCBGetSourceFromAddr( &(pBpp->addr),
        ...
        //
        p = rgch;
        if ( p = (PSTR) strchr( (LPCSTR) p,',' ) ) {
            p++;
            q = (PSTR) strchr( (LPCSTR) p,',' );
            *q = '\0';
