...
void  SetDialogPos (BOOL forced, HWND hDlg);
void  RefreshView (int startingCol,int endingCol,BOOL selectFoundText);
char *REmalloc (size_t size);
BOOL  RESearch (LPSTR line, int *xStart, int *xEnd);
BOOL  SearchLineBack (int lineNb,int *xStart,int *xEnd,PSTR findWhat);
BOOL  FindPrev (long startingLine,int startingCol,BOOL startFromSelection,BOOL selectFoundText,BOOL errorIfNotFound,HCURSOR hSaveCursor,PSTR findWhat);
BOOL  SearchLineForw (int lineNb,int *xStart,int *xEnd,PSTR findWhat);
       ...
       )
{
    ...
    if (frMem.replaceAll) {
        ...
        frMem.lpConfirmProc = (DLGPROC)DlgConfirm;
...
void FAR  Find(void)
{
    ...
    frMem.lpFindNextProc = (DLGPROC)DlgFindNext;
