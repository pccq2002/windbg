...
int  NEAR   mini (int a, int b);
int  NEAR   maxi (int a, int b);
int  NEAR   Pos2Pix (int view, int x, long y);
int  NEAR   AdjustPosX (int x, BOOL goingRight);
VOID NEAR   CSyntax (WORD curWinColor, WORD status, WORD prevStatus);
BOOL NEAR   MarkStatusLine (int doc, WORD status, WORD prevStatus);
void NEAR   SelectLine (int view, int firstX, int lastX);
void NEAR   MarkSelectedLine (int view, int line);
void NEAR   PatchTabs (int view);
void NEAR   WriteLine (int view, int line, HDC hDC, int X, int Y, PRECT pRc, WORD prevStatus);
void NEAR   SelPosXY (int view, int X, int Y);
void NEAR   MoveRightWord (int view, BOOL Sel, int posX, int posY);
void NEAR   SortBlock (int view);
void NEAR   FindMatching (int view);
...

{
    ...
    case VK_F3:
        if (ctrlDown) {
            ...
            } else if (StartDialog(DLG_FIND, (DLGPROC)DlgFind)) {
                Find();
            }
        }
        break;
