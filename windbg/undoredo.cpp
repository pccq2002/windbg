...
void  LowLevelReadBuf (LPSTR source, long offset, LPSTR dest, WORD size, long limit);
void  LowLevelWriteBuf (LPSTR dest, long offset, LPSTR source, WORD size, long limit);
void  ReadFromBuf (LPUNDOREDOREC p, LPSTR dest, WORD size);
char  ReadCharFromStreamBuf (LPUNDOREDOREC p, WORD disp);
BOOL  WriteToBuf (int doc, LPUNDOREDOREC p, WORD disp, LPSTR source, WORD size, BOOL expanding);
BOOL  InitRecBuf (LPUNDOREDOREC p, long size, BOOL allocateMem);
...
void FAR  ReadLineFromBuf(LPUNDOREDOREC p, LPSTR dest, int *size,
...
BOOL  WriteToBuf(int doc, LPUNDOREDOREC p, WORD disp, LPSTR source,
...
BOOL FAR  CreateRecBuf(int doc, BYTE recType, long bytes)
{
...
BOOL FAR  DestroyRecBuf(int doc, WORD recType)
{
    LPDOCREC docs = &Docs[doc];
    HANDLE h;
...
BOOL FAR  ResizeRecBuf(int doc, long bytes)
{
...
BOOL FAR  OpenRec(int doc, BYTE action, int col, int line)
{
    LPDOCREC docs = &Docs[doc];
    UNDOREDOREC p;
    WORD hdrSize;
...
void FAR  CloseRec(int doc, int col, int line, BOOL keepRec)
{
    LPDOCREC docs = &Docs[doc];
    UNDOREDOREC p;
    STREAMREC st;
...
BOOL FAR  AppendToRec(int doc, LPSTR chars, int size, BOOL isLine, int *totalSize)
{
    LPDOCREC docs = &Docs[doc];
    UNDOREDOREC p;
    STREAMREC st;
...
BOOL FAR  PlayRec(int doc, WORD recType, BOOL untilUserMark, BOOL prompt)
{
...
void FAR  DumpRec(int doc, WORD recType)
{
