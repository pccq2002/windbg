    ...
    )
{
    ...
    fa.wLine1 = wLine1;
    fa.lpLine2 = lpLine2;
    hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)FatalErrorBoxThread, &fa, 0, &dw);
...
/****************************************************************************
        ...
        PURPOSE:    Converts the text form of a long value in
                    ...
                    and hex (leading 0x) are supported.  If it is
                     then decimal and hex (leading $) are
                    handled.  If it is AUTO then C is tried first
                    and  if the conversion is unsuccessful.
    ...
    )
{
    ...
    if ((wLanguage == _LANGUAGE) || (wLanguage == AUTO_LANGUAGE)) {
        // look for '$' first
        // skip whitespace
        pszTmp = szSource;
...
--*/
{
    ...
    case MEMORY_WIN:
        if (NULL == lpWinInfo) {
            if (StartDialog(DLG_MEMORY, (DLGPROC)DlgMemory) != TRUE) {
                MessageBeep (0);
                return -1;         // No Debuggee or User Cancel out.
            }
        }
